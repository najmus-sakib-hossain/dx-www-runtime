...
impl HtipDemo {
    ...
    pub fn run_demo(&mut self) -> Result<JsValue, JsValue> {
        ...
        let binary = self.create_htip_payload()
        ...
        self.engine.process_stream(&stream, &root)
        ...
        web_sys::console::log_1(&format!("  Deserialization: {:.3}ms", deserialization_time).into());
    ...
    <div class="buttons">
        ...
        // Sign and return
        writer.finish_and_sign(&self.signing_key)
    ...
    pub fn run_stress_test(&mut self) -> Result<JsValue, JsValue> {
        ...
        let binary = writer.finish_and_sign(&self.signing_key)
        ...
        web_sys::console::log_1(&format!("  Payload: {} bytes ({:.2} KB)", payload_size, payload_size as f64 / 1024.0).into());
        web_sys::console::log_1(&format!("  Serialization: {:.3}ms", serialization_time).into());
        web_sys::console::log_1(&format!("  Deserialization: {:.3}ms", deserialization_time).into());
        web_sys::console::log_1(&format!("  DOM Updates: {:.3}ms", dom_time).into());
        web_sys::console::log_1(&format!("  TOTAL: {:.3}ms ({:.0} ops/ms)", total_time, 1000.0 / total_time).into());
