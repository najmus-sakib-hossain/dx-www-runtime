...
pub extern "C" fn render() {
    unsafe {
        // Template 0: <div>
                    <h1>Counter: <!--SLOT_0--></h1>
        let node_0 = host_create_element(STR_DIV_2.as_ptr(), STR_DIV_2.len() as u32);
        host_append(0, node_0);
        // Binding: slot 0 <- count
        // TODO: host_set_text(slot_0, &raw const COUNT as *const u8, core::mem::size_of_val(&COUNT) as u32);
        // Binding: slot 1 <- () => setCount(count + 1)
        // TODO: host_set_text(slot_1, &raw const () => SETCOUNT(COUNT + 1) as *const u8, core::mem::size_of_val(&() => SETCOUNT(COUNT + 1)) as u32);
    }
}
