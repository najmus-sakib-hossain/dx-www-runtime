...
impl HtipEngine {
    ...
    pub fn process_stream(&mut self, stream: &HtipStream, root: &HtmlElement) -> Result<(), String> {
        // Load string table
        self.strings.clear();
        for i in 0.. {
    ...
    fn apply_operation(&mut self, op: &Operation, root: &HtmlElement) -> Result<(), String> {
        match op {
            ...
            Operation::Instantiate(inst) => {
                self.instantiate_template(inst.instance_id, inst.template_id, inst.parent_id, root)?;
            }
            ...
            Operation::AttachEvent(event) => {
                ...
                web_sys::console::warn_1(&format!("AttachEvent not yet implemented: {:?}", event).into());
            }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    fn patch_text(&mut self, _instance_id: u32, _slot_id: u16, _string_id: u32) -> Result<(), String> {
        Ok(()) // Mock
    }
    ...
    /// Toggle a CSS class
    #[cfg(target_arch = "wasm32")]
    fn toggle_class(&mut self, instance_id: u32, class_name_id: u32, enabled: bool) -> Result<(), String> {
        let instance = self.instances
            .get(&instance_id)
            .ok_or_else(|| format!("Instance {} not found", instance_id))?;
        ...
        if enabled {
            class_list.add_1(class_name).map_err(|e| format!("Failed to add class: {:?}", e))?;
        } else {
            class_list.remove_1(class_name).map_err(|e| format!("Failed to remove class: {:?}", e))?;
        }
    ...
    fn remove_node(&mut self, instance_id: u32) -> Result<(), String> {
        if let Some(instance) = self.instances.remove(&instance_id) {
            if let Some(parent) = instance.parent_node() {
                parent.remove_child(&instance).map_err(|e| format!("Failed to remove: {:?}", e))?;
            }
        }
        Ok(())
    }
    ...
    fn append_child(&mut self, parent_id: u32, child_id: u32) -> Result<(), String> {
        let parent = self.instances
            .get(&parent_id)
            .ok_or_else(|| format!("Parent {} not found", parent_id))?
            .clone();
        
        let child = self.instances
            .get(&child_id)
