...
impl Renderer {
    ...
    pub fn process_stream(&mut self, data: &[u8], header: &HtipHeader) -> Result<(), u8> {
        ...
        if header.template_count > 0 {
            for _ in 0..header.template_count {
                ...
                let entry = unsafe {
                    ptr::read_unaligned(
                        data.as_ptr().add(offset) as *const TemplateEntry
                    )
                };
        ...
        for _ in 0..header.opcode_count {
            ...
            let op_header = unsafe {
                ptr::read_unaligned(
                    data.as_ptr().add(offset) as *const OpcodeHeader
                )
            };
    ...
    fn execute_op(
        ...
        let op_type = OpType::from_u8(header.op_type)
            .ok_or(ErrorCode::InvalidOpcode as u8)?;
    ...
    fn execute_patch_text(
        ...
        let text = strings.get(payload.string_idx)
    ...
    fn execute_patch_attr(
        ...
        let name = strings.get(payload.attr_name_idx)
            .ok_or(ErrorCode::StringIndexOutOfBounds as u8)?;
        let value = strings.get(payload.attr_value_idx)
            .ok_or(ErrorCode::StringIndexOutOfBounds as u8)?;
    ...
    fn execute_class_toggle(
        ...
        let class_name = strings.get(payload.class_name_idx)
            .ok_or(ErrorCode::StringIndexOutOfBounds as u8)?;
    ...
    fn execute_set_style(
        ...
        let prop = strings.get(payload.prop_name_idx)
            .ok_or(ErrorCode::StringIndexOutOfBounds as u8)?;
        let value = strings.get(payload.prop_value_idx)
            .ok_or(ErrorCode::StringIndexOutOfBounds as u8)?;
