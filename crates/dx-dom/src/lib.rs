...
use web_sys::{window, Document, HtmlTemplateElement, DocumentFragment, Node};
...
pub fn register_templates(binary_data: &[u8]) {
    ...
    for i in 0..count {
        ...
        let html = std::str::from_utf8(html_bytes)
            .expect("Invalid UTF-8 in template HTML");
        offset += html_length;
    ...
    pub fn flush(&mut self) {
        ...
        for op in &self.pending_ops {
            match op.opcode {
                ...
                _ => {
                    web_sys::console::warn_1(&format!("Unknown opcode: {}", op.opcode).into());
                }
            }
        }
